...
// Sample data - moved to top for better organization
const BLOG_DATA = {
  recentPosts: [
    {
      id: 1,
  ...
  categories: [
    { name: 'Food and Cuisine', count: 15 },
    { name: 'Travel Stories', count: 8 },
    { name: 'Lifestyle', count: 12 },
    { name: 'Food and Wine', count: 20 },
    { name: 'Events and Entertainment', count: 6 }
  ],
  popularTags: ['RESTAURANT', 'FOOD', 'WINE', 'TRAVEL', 'LIFESTYLE', 'CULTURE'],
  mainArticle: {
    category: 'LIFESTYLE',
    title: 'THE WORLD OF FOOD AND WINE MATCHING',
    date: '18 JUL 2023',
    author: 'Author Name',
    stats: {
      views: 1234,
  ...
  <div className="mt-8">
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {[1, 2, 3].map((index) => (
        <div 
          key={index} 
          className="aspect-square bg-gray-200 rounded-lg hover:opacity-80 cursor-pointer transition-opacity"
        >
  ...
  return (
    <article className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-6">
        ...
        <ArticleMeta date={mainArticle.date} author={mainArticle.author} />
        <ArticleContent />
        <RelatedImages />
        <ArticleStats stats={mainArticle.stats} />
      </div>
    </article>
  );
};
...
const Categories = () => (
  <div className="bg-white rounded-lg shadow-md p-6">
    ...
    <div className="space-y-2">
      {BLOG_DATA.categories.map((category, index) => (
        <div 
          key={index} 
  ...
  <div className="bg-white rounded-lg shadow-md p-6">
    ...
    <div className="grid grid-cols-3 gap-2">
      {Array.from({ length: 9 }, (_, index) => (
        <div 
          key={index} 
          className="aspect-square bg-gray-200 rounded hover:opacity-80 cursor-pointer transition-opacity"
        >
        </div>
      ))}
    </div>
...
// Popular Tags Sidebar Component
const PopularTags = () => (
  <div className="bg-white rounded-lg shadow-md p-6">
    <h3 className="text-lg font-bold text-gray-900 mb-4 border-b pb-2">
      POPULAR TAGS
    ...
    <div className="flex flex-wrap gap-2">
      {BLOG_DATA.popularTags.map((tag, index) => (
        <span 
          key={index} 
          className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-xs hover:bg-red-500 hover:text-white cursor-pointer transition-colors"
        >
          {tag}
        </span>
      ))}
